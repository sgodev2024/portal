                    ...
                    @php
                        ...
                        if (isset($msg['sender_id'])) {
                            if ($chat->user && $msg['sender_id'] == $chat->user->id) {
                                $sender = $chat->user->name ?? 'Khách hàng';
                            } elseif ($chat->staff && $msg['sender_id'] == $chat->staff->id) {
                                $sender = $chat->staff->name ?? 'Nhân viên';
                    ...
                    <div class="message-wrapper {{ $isMe ? 'sent' : 'received' }}" data-created-at="{{ $msg['created_at'] ?? '' }}">
                        ...
                        <div class="message-content">
                            ...
                            {{-- <div class="message-meta">
                                {{ $sender }} • {{ isset($msg['created_at']) ? \Carbon\Carbon::parse($msg['created_at'])->format('H:i d/m/Y') : '' }}
                            </div> --}}
                        </div>
                    </div>
                @endforeach
                ...
                <form class="chat-input-form" id="chatForm" method="POST"
                    ...
                    <div class="input-area">
                        ...
                        <textarea name="message" class="message-input"
                                  placeholder="Aa" rows="1" id="messageInput"></textarea>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function appendMessages(messages) {
        ...
        messages.forEach(msg => {
            ...
            const sender = isMe ? '{{ auth()->user()->name }}' : '{{ $chat->user->name ?? "Khách hàng" }}';
            ...
            if (msg.type === 'text') {
                ...
                content = `<a href="/storage/${msg.file_path}" target="_blank">
