            ...
            <div class="chat-list">
                ...
                <div class="tab-content-list active" data-content="pending">
                    @forelse($pendingChats as $chat)
                        <div class="chat-item" data-chat-id="{{ $chat->id }}" data-status="pending" data-user-name="{{ $chat->user->name ?? 'Khách hàng' }}">
                ...
                <div class="tab-content-list" data-content="processing" style="display: none;">
                    @forelse($processingChats as $chat)
                        ...
                        @endphp
                        <div class="chat-item" data-chat-id="{{ $chat->id }}" data-status="processing" data-user-name="{{ $chat->user->name ?? 'Khách hàng' }}" data-staff-name="{{ $chat->staff->name ?? 'Nhân viên' }}">
                            <div class="chat-avatar">
                                {{ strtoupper(substr($chat->user->name ?? 'U', 0, 1)) }}
                            </div>
                            <div class="chat-info">
...
$(document).ready(function() {
    ...
    function renderMessages(messages, staffName) {
        ...
        messages.forEach(msg => {
            ...
            const senderName = isMe ? '{{ auth()->user()->name }}' : (staffName || 'Khách hàng');
            ...
            if (msg.type === 'text') {
                ...
                content = `<a href="/storage/${msg.file_path}" target="_blank">📎 ${msg.file_name || 'Tệp đính kèm'}</a>`;
            }
    ...
    chatForm.on('submit', function(e) {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                ...
                alert('Lỗi kết nối khi gửi tin nhắn: ' + (xhr.responseJSON?.message || 'Unknown error'));
            },
            complete: function() {
