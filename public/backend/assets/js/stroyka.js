        ...
        if ($sidebarBody.length > 0) {
            ...
            if (itemsState) {
                ...
                itemsState.forEach(function (path) {
                    const itemQuery = path.slice(1).map(function (itemIndex) {
                        return (
                            ".sa-nav__menu > .sa-nav__menu-item:eq(" +
                            itemIndex +
                            ")"
                        );
                    });
            ...
            $sidebarBody.on("sa-collapse-state", function () {
                function getMenuState(parentElement, path) {
                    ...
                    $(
                        "> .sa-nav__menu > .sa-nav__menu-item",
                        parentElement
                        ...
                        result = result.concat(
                            getMenuState(element, currentPath)
                        );
                    });
    ...
    */
    $("[data-sa-collapse]").each(function (i, element) {
        ...
        $triggers.on("click", function (event) {
            ...
            const $content = $item
                .find("[data-sa-collapse-content]")
                .filter(function (index, element) {
                    return $(element)
                        .closest("[data-sa-collapse-item]")
                        .is($item);
                });
            const $parents = $item.parents();
            const openClass = $item.data("sa-collapse-item");
            $parents
                .slice(0, $parents.index(collapse) + 1)
                .filter("[data-sa-collapse-item]")
                .css("height", "");
        ...
        $(".sa-search").each(function (i, element) {
            ...
            const search = function () {
                ...
                const requestMiddleware =
                    window.stroyka.search.requestMiddleware;
            ...
            const setState = function (state) {
                $search.toggleClass(
                    "sa-search--state--pending",
                    state === "pending"
                );
                $search.toggleClass(
                    "sa-search--state--has-results",
                    state === "has-results"
                );
                $search.toggleClass(
                    "sa-search--state--no-results",
                    state === "no-results"
                );
            };
            ...
            $(document).on("focusin", function () {
                ...
                if (
                    !$search.is($(document.activeElement).closest(".sa-search"))
                ) {
                    blur();
                }
            });
            ...
            request: function (query, abortController) {
                return new Promise(function (resolve, reject) {
                    const settings = Object.assign(
                        {},
                        window.stroyka.search.getAjaxSettings(query),
                        {
        ...
        const ro = new ResizeObserver(function (entries) {
            ...
            entries.forEach(function (entry) {
                const breakpoints = JSON.parse(
                    entry.target.dataset.saContainerQuery
                );
                ...
                Object.keys(breakpoints)
                    ...
                    .forEach(function (width) {
                        ...
                        if (entry.borderBoxSize) {
                            const borderBoxSize = Array.isArray(
                                entry.borderBoxSize
                            )
                ...
                tasks.push(function () {
                    entry.target.classList.remove.apply(
    ...
    window.stroyka.inboxList = (function () {
        const inboxList = {
            init: function (context) {
                ...
                $context
                    .find(".sa-inbox-list__item")
                    .on("mouseover", function (event) {
                        const isInteractiveElement =
                            inboxList.getClosestInteractiveElement(event.target)
                                .length !== 0;

                        $(this).toggleClass(
                            "sa-inbox-list__item--hover",
            ...
            getClosestInteractiveElement: function (target) {
                return $(target).closest(
                    ".sa-inbox-list__star, .sa-inbox-list__checkbox"
                );
            },
        };
    ...
    window.stroyka.inboxChat = (function () {
        const inboxChat = {
            init: function (context) {
                ...
                $context
                    .find(".sa-inbox-chat__item-header")
                    .on("click", function (event) {
                        const isInteractiveElement =
                            inboxChat.getClosestInteractiveElement(event.target)
                                .length !== 0;
                        ...
                        $(this)
                            .closest(".sa-inbox-chat__item")
                            .toggleClass("sa-inbox-chat__item--expanded");
                    });
            },
            getClosestInteractiveElement: function (target) {
    ...
    */
    window.stroyka.inboxSidebar = (function () {
        ...
        return {
            toggle: function (value) {
                const curState =
                    $inbox.hasClass(openClass) && !$inbox.hasClass(openClass);
                const newState = typeof value === "boolean" ? value : !curState;
    ...
    */
    window.stroyka.chat = (function () {
        ...
        return {
            toggle: function (value) {
                const curState =
                    $chat.hasClass(openClass) && !$chat.hasClass(openClass);
                const newState = typeof value === "boolean" ? value : !curState;
    ...
    window.stroyka.colors = (function () {
        return {
            getThemeColor: function () {
                ...
                ).getPropertyValue("--sa-scheme-theme--main-color");
            },
        };
    })();
    ...
    */
    window.stroyka.nouislider = (function () {
        const calcPipsSize = function (slider) {
            const size = [].slice
                .call(slider.target.querySelectorAll(".noUi-value"))
            ...
            slider.target.style.setProperty(
                "--sa-nouislider-pips--max-width",
                size.width + "px"
            );
            slider.target.style.setProperty(
                "--sa-nouislider-pips--max-height",
                size.height + "px"
            );
        };
